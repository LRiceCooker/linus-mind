...
- [ ] Fix case-insensitive deduplication in `js/wikipedia.js`. Currently `candidates` is a Set that stores words as-is — so "UTF8" and "utf8" on the same page are treated as different candidates, one gets a link and the other doesn't. Fix: normalize candidates to a canonical form for dedup. Use the FIRST occurrence's casing for display but deduplicate by lowercase. When `wrapWordInLink` is called, it should find ALL occurrences of the word (case-insensitive) and wrap them all, not just the first one.
